...
`captain.invitation_redeemed` (task T4.1 reserved the three invitation kinds;
task T4.2 writes them. `captain.assigned` and `captain.unassigned` remain
unwritten until task T4.4). They are the
