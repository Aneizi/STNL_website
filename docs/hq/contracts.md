...
| CRM person identity | stable person id, account link, edition People references, explicit correction | new `lib/hq/crm-identity.ts`, `lib/hq/queries.ts`, `lib/hq/actions/people.ts` | `ensurePersonForAccount(userId, name)`, `ensurePersonForRosterMember({ colosseumUsername, displayName })`, `linkPersonToAccount(personId, userId, actor)`, `correctPersonMatch({ personId, fromUserId, toUserId, actor, reason })` | 1, used from 3 |
