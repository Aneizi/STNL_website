...
// the operator recorded on every row and event, grant and event committing or
// rolling back together, and an audit module that cannot update or delete.
...
describe("grantCapability and revokeCapability", () => {
  it("grants once, records the operator and the reason, and is visible on the next read", async () => {
    const grant = await grantCapability(db, { actorOperatorId: OPERATOR, userId: "acct-a", capability: "captain", reason: "Leads the Utrecht cohort" });
  ...
  it("returns the existing grant on a repeat and writes no second row or event", async () => {
    const first = await grantCapability(db, { actorOperatorId: OPERATOR, userId: "acct-a", capability: "captain", reason: "first" });
    const again = await grantCapability(db, { actorOperatorId: SECOND_OPERATOR, userId: "acct-a", capability: "captain", reason: "second" });
  ...
  it("revokes once, keeps the row as history, and lets a later grant start a new row", async () => {
    const grant = await grantCapability(db, { actorOperatorId: OPERATOR, userId: "acct-a", capability: "captain", reason: "start" });
    const revoked = await revokeCapability(db, { actorOperatorId: SECOND_OPERATOR, userId: "acct-a", capability: "captain", reason: "moved on" });
    ...
    expect(await revokeCapability(db, { actorOperatorId: OPERATOR, userId: "acct-a", capability: "captain", reason: "again" })).toBeNull();
    expect(await events()).toEqual([
    ...
    const regrant = await grantCapability(db, { actorOperatorId: OPERATOR, userId: "acct-a", capability: "captain", reason: "back" });
  ...
  it("allows one effective grant per capability at the database level", async () => {
    await grantCapability(db, { actorOperatorId: OPERATOR, userId: "acct-a", capability: "captain", reason: "x" });
  ...
  it("rolls the grant back when the audit event cannot be written", async () => {
    ...
    try {
      await expect(grantCapability(db, { actorOperatorId: OPERATOR, userId: "acct-a", capability: "captain", reason: "x" })).rejects.toThrow(/test_audit_failure/);
    ...
    try {
      await expect(revokeCapability(db, { actorOperatorId: OPERATOR, userId: "acct-a", capability: "captain", reason: "y" })).rejects.toThrow(/test_audit_failure/);
    } finally { await rows("ALTER TABLE hq_audit_events DROP CONSTRAINT test_audit_failure"); }
    expect(await listActiveCapabilities("acct-a")).toEqual(["captain"]);
    expect(await events()).toHaveLength(1);
  });
  ...
  it("refuses an unknown account or capability without writing anything", async () => {
    await expect(grantCapability(db, { actorOperatorId: OPERATOR, userId: "nobody", capability: "captain", reason: "x" })).rejects.toThrow("account");
    await expect(grantCapability(db, { actorOperatorId: OPERATOR, userId: "acct-a", capability: "mayor" as "captain", reason: "x" })).rejects.toThrow("capability");
    expect(await grantRows()).toEqual([]);
    expect(await events()).toEqual([]);
  });
  ...
  it("joins the caller's transaction, so a failing later statement takes the grant with it", async () => {
    await expect(db.transaction(async (tx) => {
      await grantCapability(tx, { actorOperatorId: OPERATOR, userId: "acct-a", capability: "captain", reason: "x" });
...
describe("reading capabilities", () => {
  it("answers for every requested account in one query and skips the query for none", async () => {
    await grantCapability(db, { actorOperatorId: OPERATOR, userId: "acct-b", capability: "captain", reason: "x" });
