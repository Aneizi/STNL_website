...
export default async function InitializePage({searchParams}:{searchParams:Promise<{hackathon?:string}>}){
  ...
  const user=await requireMember(`/hq/initialize${hackathon?`?hackathon=${encodeURIComponent(hackathon)}`:''}`);
  const [editions,telegramConnected]=await Promise.all([builderStore().hackathons(),hasTelegramIdentity(user.id)]);
  ...
  return <BuilderShell back='/hq/welcome'><h1>Initialize your <em>team.</em></h1><BuilderInitialize hackathon={edition} available={available} telegramConnected={telegramConnected}/></BuilderShell>;
}
